...
## Record-Level Encryption (PIN-based)
Ngoài mã hóa file-level (.db.enc), hệ thống còn hỗ trợ **mã hóa record-level** dựa trên PIN code của bác sĩ:
- Mỗi bác sĩ có PIN 6 số riêng
- Dữ liệu nhạy cảm (kết quả xét nghiệm, chẩn đoán) được mã hóa bằng key derive từ PIN
- Chỉ người có PIN mới có thể giải mã và xem dữ liệu
- Phù hợp cho môi trường offline khi không có kết nối mạng để xác thực

## Related Documents
- [07. Active Sync Protocol](07_active_sync_protocol.md)
- [13. AWARE-SAVE Protocol](13_Aware_Save_Protocol.md)
